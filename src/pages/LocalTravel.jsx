...
import { MapPin, Loader2, Upload, Eye, Plus, Trash2, ArrowLeft } from "lucide-react";
...
const checkPendingOutFromServer = async () => {
    try {
        console.log("Checking for pending OUT forms from server for user:", salesPersonName);
        ...
        console.log("Server check request sent for pending OUT forms");
   ...
   const hasPendingOutForm = () => {
    ...
    console.log("hasPendingOutForm check for user:", salesPersonName);
    console.log("Pending data exists:", hasPending);
  ...
  useEffect(() => {
    const checkAndEnforcePendingOutForm = async () => {
        ...
        console.log("Checking for pending OUT form for user:", salesPersonName);
        console.log("Local pending OUT data found:", pendingOutData);
        
        // If no local data, check if there might be incomplete entries on server
        if (!pendingOutData) {
            console.log("No local pending data, checking server for incomplete entries...");
            ...
            const possiblePendingData = sessionStorage.getItem(`serverPending_${salesPersonName}`);
            if (possiblePendingData) {
                console.log("Found session pending data from server check");
        ...
        if (pendingOutData) {
            try {
                ...
                console.log("Found pending OUT form - forcing OUT section:", parsedData);
            ...
            console.log("No pending OUT form found - allowing new IN form");
...
const generateSerialNumber = async () => {
    try {
        ...
        if (rows.length === 0) {
            console.log("No data rows found, starting with TI-001");
            return { serialNumber: "TI-001", count: 1 };
        }
        ...
        console.log("All serial numbers found:", formattedHistory.map(item => item.serialNumber));

        if (formattedHistory.length === 0) {
            console.log("No valid serial numbers found, starting with TI-001");
            return { serialNumber: "TI-001", count: 1 };
        }
     ...
     setLocalTravels(formattedHistory);
        ...
        console.log("Valid TI numbers found:", tiNumbers);

        if (tiNumbers.length === 0) {
            console.log("No valid TI numbers found, starting with TI-001");
            return { serialNumber: "TI-001", count: 1 };
        }
        ...
        // console.log("Highest TI number found:", highestNumber);
...
useEffect(()=>{
    ...
    const validateInForm = () => {
        ...
        if (formData.inVehicleType && (formData.inVehicleType === "Car" || formData.inVehicleType === "Bike")) {
        ...
        if (formData.inVehicleType === "Rent Car" || formData.inVehicleType === "Rent Bike") {
    ...
    const validateOutForm = () => {
        ...
        if (formData.outVehicleType === "Car" || formData.outVehicleType === "Bike") {
            if (!formData.outVehicleMeterNumber) {
                newErrors.outVehicleMeterNumber = "OUT vehicle meter number is required";
            }
            ...
            if (outMeter > 0 && inMeter > 0 && outMeter <= inMeter) {
                newErrors.outVehicleMeterNumber = "OUT meter number should be greater than IN meter number";
            }
        }
        ...
        if (formData.outVehicleType === "Rent Car" || formData.outVehicleType === "Rent Bike") {
...
const handleInSubmit = async (e) => {
  ...
  try {
    ...
    if (formData.inVehicleMeterImage) {
      try {
        const fileName = `in_vehicle_meter_${salesPersonName}_${formData.travelDate}.${formData.inVehicleMeterImage.name.split('.').pop()}`;
        await uploadFileToGoogleDrive(formData.inVehicleMeterImage, fileName);
        vehicleImageLinks.push(fileName);
        console.log("IN vehicle meter image uploaded");
      } catch (uploadError) {
    ...
    if (formData.inBusTicketImage) {
      try {
        const fileName = `in_bus_ticket_${salesPersonName}_${formData.travelDate}.${formData.inBusTicketImage.name.split('.').pop()}`;
        await uploadFileToGoogleDrive(formData.inBusTicketImage, fileName);
        vehicleImageLinks.push(fileName);
        console.log("IN bus ticket image uploaded");
      } catch (uploadError) {
    ...
    if (formData.inBillReceipt) {
      try {
        const fileName = `in_bill_receipt_${salesPersonName}_${formData.travelDate}.${formData.inBillReceipt.name.split('.').pop()}`;
        await uploadFileToGoogleDrive(formData.inBillReceipt, fileName);
        vehicleImageLinks.push(fileName);
        console.log("IN bill receipt uploaded");
      } catch (uploadError) {
    ...
    rowData[FMS_COLUMNS.IN_METER_NUMBER] = formData.inVehicleMeterNumber || "";
    ...
    // console.log("IN data to submit:", rowData);
    ...
    showToast("IN travel information submitted successfully! अब OUT form fill करें।", "success");
    ...
    sessionStorage.setItem(`serverPending_${salesPersonName}`, JSON.stringify(pendingOutData));
...
const handleOutSubmit = async (e) => {
  ...
  try {
    ...
    if (formData.outVehicleMeterImage) {
      try {
        const fileName = `out_vehicle_meter_${salesPersonName}_${formData.returnDate}.${formData.outVehicleMeterImage.name.split('.').pop()}`;
        await uploadFileToGoogleDrive(formData.outVehicleMeterImage, fileName);
    ...
    if (formData.outBusTicketImage) {
      try {
        const fileName = `out_bus_ticket_${salesPersonName}_${formData.returnDate}.${formData.outBusTicketImage.name.split('.').pop()}`;
    ...
    if (formData.outBillReceipt) {
      try {
        const fileName = `out_bill_receipt_${salesPersonName}_${formData.returnDate}.${formData.outBillReceipt.name.split('.').pop()}`;
...
console.log("Updating serial number:", serialNumberToUpdate);
    ...
    showToast("Complete travel information submitted successfully!", "success");
                    ...
                    {!showOutSection && !hasPendingOutForm() && (
                        <form onSubmit={handleInSubmit} className="space-y-8 p-8">
                            <div className="bg-blue-50 rounded-xl p-6">
                                <h4 className="text-lg font-semibold text-blue-800 mb-4">IN Travel Details</h4>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div className="relative">
                                                <input
                                                    type="file"
                                                    accept="image/*"
                                                    onChange={(e) => handleFileChange(e, "inVehicleMeter")}
                                                    className="hidden"
                                                ...
                                                >
                                                    <div className="text-center">
                                                        ...
                                                        <p className="text-slate-600 text-sm">
                                                            {formData.inVehicleMeterImageName || "Upload meter image"}
                                                        </p>
                                                    </div>
                                                </label>
                                            </div>
                                            ...
                                            <div className="relative">
                                                ...
                                                >
                                                    <div className="text-center">
                                                        <Upload className="h-6 w-6 text-slate-400 mx-auto mb-1" />
                                                        <p className="text-slate-600 text-sm">
                                                            {formData.inBusTicketImageName || "Upload bus ticket"}
                                                        </p>
                                                    </div>
                                                </label>
                                            </div>
                                ...
                                {(formData.inVehicleType === "Rent Car" || formData.inVehicleType === "Rent Bike") && (
                                    <div className="grid gap-4 lg:grid-cols-2 mb-4">
                                        <div className="space-y-2">
                                            <label className="block text-sm font-semibold text-slate-700 mb-3">
                                                Bill Receipt
                                            ...
                                            <div className="relative">
                                                ...
                                                >
                                                    <div className="text-center">
                                                        <Upload className="h-6 w-6 text-slate-400 mx-auto mb-1" />
                                                        <p className="text-slate-600 text-sm">
                                                            {formData.inBillReceiptName || "Upload bill receipt"}
                                                        </p>
                                                    </div>
                                                </label>
                                            </div>
                        ...
                        <form onSubmit={handleOutSubmit} className="space-y-8 p-8">
                            <div className="bg-green-50 rounded-xl p-6">
                                <div className="flex justify-between items-center mb-4">
                                    <h4 className="text-lg font-semibold text-green-800">OUT Travel Details</h4>
                                ...
                                {(formData.outVehicleType === "Car" || formData.outVehicleType === "Bike") && (
                                    <div className="space-y-4 mb-4">
                                        ...
                                        <div className="grid gap-4 lg:grid-cols-2">
                                            <div className="space-y-2">
                                                <label className="block text-sm font-semibold text-slate-700 mb-3">
                                                    Vehicle Meter Number (OUT) <span className="text-red-500">*</span>
                                                </label>
                                                <input
                                                    type="text"
                                                ...
                                                {formData.outVehicleMeterNumber && submittedInData?.inVehicleMeterNumber && (
                                                    <div className="bg-blue-100 p-3 rounded-lg">
                                                        <p className="text-sm text-blue-800 font-medium">
                                                            Total Running KM: {calculateTotalRunningKm(submittedInData.inVehicleMeterNumber, formData.outVehicleMeterNumber)} km
                                                        </p>
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <div className="relative">
                                                    <input
                                                        type="file"
                                                        accept="image/*"
                                                        onChange={(e) => handleFileChange(e, "outVehicleMeter")}
                                                        className="hidden"
                                                    ...
                                                    >
                                                        <div className="text-center">
                                                            <Upload className="h-6 w-6 text-slate-400 mx-auto mb-1" />
                                                            <p className="text-slate-600 text-sm">
                                                                {formData.outVehicleMeterImageName || "Upload OUT meter image"}
                                                            </p>
                                                        </div>
                                                    </label>
                                                </div>
                                            ...
                                            <div className="relative">
                                                ...
                                                >
                                                    <div className="text-center">
                                                        <Upload className="h-6 w-6 text-slate-400 mx-auto mb-1" />
                                                        <p className="text-slate-600 text-sm">
                                                            {formData.outBusTicketImageName || "Upload bus ticket"}
                                                        </p>
                                                    </div>
                                                </label>
                                            </div>
                                ...
                                {/* Rent Car/Bike Fields */}
                                {(formData.outVehicleType === "Rent Car" || formData.outVehicleType === "Rent Bike") && (
                                    <div className="grid gap-4 lg:grid-cols-2 mb-4">
                                        <div className="space-y-2">
                                            <label className="block text-sm font-semibold text-slate-700 mb-3">
                                                Bill Receipt
                                            ...
                                            <div className="relative">
                                                <input
                                                    type="file"
                                                    accept="image/*"
                                                    onChange={(e) => handleFileChange(e, "outBillReceipt")}
                                                    className="hidden"
                                                ...
                                                >
                                                    <div className="text-center">
                                                        <Upload className="h-6 w-6 text-slate-400 mx-auto mb-1" />
                                                        <p className="text-slate-600 text-sm">
                                                            {formData.outBillReceiptName || "Upload bill receipt"}
                                                        </p>
                                                    </div>
                                                </label>
                                            </div>
