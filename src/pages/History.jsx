...
import { MapPin, Eye, Loader2, ChevronDown, ChevronUp } from "lucide-react";
  ...
  const fetchTravelHistory = async () => {
    ...
    try {
      ...
      const formattedHistory = rows.map((row, index) => {
        ...
        const personName = row.c?.[1]?.v; // Column B - Person Name
        const fromLocation = row.c?.[2]?.v; // Column C - From Location
        const toLocation = row.c?.[3]?.v; // Column D - To Location
        const vehicleType = row.c?.[4]?.v; // Column E - Vehicle Type
        const noOfDays = row.c?.[5]?.v; // Column F - Number of Days
        const accommodationType = row.c?.[6]?.v; // Column G - Accommodation Type
        const travelDate = row.c?.[7]?.v; // Column H - Travel Date
        const stayBillLink = row.c?.[8]?.v; // Column I - Stay Bill Link
        const foodingDates = row.c?.[9]?.v; // Column J - Fooding Dates
        const foodingBillLinks = row.c?.[10]?.v; // Column K - Fooding Bill Links
        const travelReceiptLink = row.c?.[11]?.v; // Column L - Travel Receipt
        const localTravelType = row.c?.[12]?.v; // Column M - Local Travel Type
        const localTravelReceiptLink = row.c?.[13]?.v; // Column N - Local Travel Receipt
        const returnDate = row.c?.[14]?.v; // Column O - Return Date
        const returnTicketLink = row.c?.[15]?.v; // Column P - Return Ticket
        const advanceAmount = row.c?.[16]?.v; // Column Q - Advance Amount
          ...
          <div className="p-6">
            {isLoadingHistory ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-slate-500 text-lg font-medium">No travel records found</p>
                <p className="text-slate-400 mt-2">Your travel submissions will appear here</p>
              </div>
            ) : (
